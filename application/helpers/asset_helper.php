		...
		if ($this->_count_js) {
			...
			foreach ($this->_files['javascript'] as $javascript) {
				if (file_exists($this->_js_dir.$javascript.'.js'))
					$temp .= '<script type="text/javascript" src="'.JS_URL.'/'.$javascript.'.js?t='.time().'" charset="'.CHAR_SET.'"></script>'."\n";
			}
			return $temp;
		} else {
			return null;
		...
		if ($this->_count_css) {
			...
			foreach ($this->_files['css'] as $css) {
				if (file_exists($this->_css_dir.$css.'.css'))
					$temp .= '<link href="'.CSS_URL.'/'.$css.'.css?t='.time().'" rel="stylesheet" type="text/css" media="all" />'."\n";
			}
			return $temp;
		} else {
			return null;
